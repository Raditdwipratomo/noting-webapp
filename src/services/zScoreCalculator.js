...
class ZScoreCalculator {
  ...
  static async calculateHeightForAge(tinggiCm, jenisKelamin, usiaBulan) {
    try {
      ...
      if (!standar) {
        throw new Error(
          `Standar WHO tidak ditemukan untuk usia ${usiaBulan} bulan`,
        );
      }

      // Tentukan SD yang sesuai berdasarkan posisi nilai
      let sd = 0;
      if (Number(tinggiCm) < Number(standar.tb_median)) {
        // Gunakan SD negatif
        sd = Number(standar.tb_median) - Number(standar.tb_minus_1sd);
        console.log(
          "Ops",
          Number(standar.tb_median) - Number(standar.tb_minus_1sd),
          standar.tb_median,
          standar.tb_minus_1sd,
        );
      } else {
        // Gunakan SD positif
        sd = Number(standar.tb_plus_1sd) - Number(standar.tb_median);
      }

      const zScore = (Number(tinggiCm) - Number(standar.tb_median)) / sd;
      ...
      console.log(
        ...
        "standar",
        "test",
        Number(tinggiCm) < Number(standar.tb_median),
      );

      console.log("z score for this ", zScore);
